...
Har jawab mein:
1. Pehle Qadeer bhai ka kaam samjho
2. Detailed aur useful output do
3. End mein ek friendly note ya tip zaroor lagao
4. Emojis use karo magar zyada nahi`;
...
function TypingDots() {
  return (
    <div style={{ display: "flex", gap: 5, padding: "14px 18px", alignItems: "center" }}>
      ...
      <span style={{ fontSize: 12, color: "#94a3b8", marginLeft: 4, fontFamily: "'DM Sans', sans-serif" }}>
        Soch raha hoon...
      </span>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      background: isUser
        ...
        : "linear-gradient(135deg, #c026d3, #818cf8, #06b6d4)",
      display: "flex", alignItems: "center", justifyContent: "center",
      fontSize: 17, marginTop: 2,
      boxShadow: isUser ? "0 0 16px rgba(14,165,233,0.4)" : "0 0 16px rgba(192,38,211,0.45)",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: isUser ? "1px solid rgba(139,92,246,0.4)" : "1px solid rgba(255,255,255,0.09)",
        color: "#f1f5f9", fontSize: 14, lineHeight: 1.75,
        whiteSpace: "pre-wrap", wordBreak: "break-word",
        backdropFilter: isUser ? "none" : "blur(12px)",
        boxShadow: isUser ? "0 8px 24px rgba(124,58,237,0.35)" : "0 4px 16px rgba(0,0,0,0.2)",
...
export default function App() {
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#07080f", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position:"absolute", width:300, height:300, borderRadius:"50%", background:"#06b6d4", top:"40%", right:"20%", filter:"blur(80px)", opacity:0.1, animation:"orbFloat 12s ease-in-out infinite reverse", pointerEvents:"none" }} />
      ...
      <div style={{ position:"relative", zIndex:10, padding:"16px 22px", borderBottom:"1px solid rgba(255,255,255,0.07)", background:"rgba(7,8,15,0.75)", backdropFilter:"blur(24px)", display:"flex", alignItems:"center", gap:14 }}>
        ...
        <div style={{ textAlign:"right", fontSize:11, color:"#64748b", lineHeight:1.6 }}>
          <div style={{ color:"#a78bfa" }}>Marketing & Content</div>
          <div>Strategy & Research</div>
        </div>
      </div>

      <div style={{ position:"relative", zIndex:10, padding:"10px 18px", borderBottom:"1px solid rgba(255,255,255,0.05)", display:"flex", gap:7, overflowX:"auto", scrollbarWidth:"none", background:"rgba(7,8,15,0.5)", backdropFilter:"blur(16px)" }}>
        {QUICK_TASKS.map((t, i) => (
      ...
      <div style={{ position:"relative", zIndex:10, padding:"14px 18px 18px", background:"rgba(7,8,15,0.75)", backdropFilter:"blur(24px)", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ position:"absolute", top:0, left:"10%", right:"10%", height:1, background:"linear-gradient(90deg,transparent,rgba(192,38,211,0.5),rgba(129,140,248,0.5),transparent)" }} />
        <div style={{ display:"flex", gap:10, alignItems:"flex-end", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(192,38,211,0.3)", borderRadius:18, padding:"11px 14px", boxShadow:"0 0 30px rgba(192,38,211,0.08)" }}>
          <textarea
            ref={textareaRef}
            value={input}
            ...
            placeholder="Qadeer bhai, kya kaam hai aaj? ✨"
            rows={1}
            style={{ flex:1, background:"transparent", border:"none", outline:"none", color:"#f1f5f9", fontSize:14, lineHeight:1.6, fontFamily:"sans-serif", resize:"none", maxHeight:130, overflowY:"auto" }}
          />
          <button className="sbtn" onClick={() => send()} disabled={loading || !input.trim()} style={{ width:42, height:42, borderRadius:"50%", border:"none", cursor: loading || !input.trim() ? "not-allowed" : "pointer", background: loading || !input.trim() ? "rgba(192,38,211,0.2)" : "linear-gradient(135deg,#c026d3,#818cf8)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0, boxShadow: !loading && input.trim() ? "0 0 20px rgba(192,38,211,0.45)" : "none" }}>
            {loading ? "⏳" : "🚀"}
          </button>
        </div>
